...
class strategy_marubozu(Strategy):
    ...
    def strategy_body():
        ...
        data['longline'] = ta.CDLLONGLINE(data['Open'], data['High'], data['Low'], data['Close'])

        # Buy/Sell
        data['signal'] = 0
        for i in range(len(data)):
            if (data['marubozu'][i] == 100 or data['longline'][i] == 100) and data['Close'][i] > data['MBB'][i] and data['Low'][i] < data['MBB'][i]:
                data['signal'][i] = 100
            elif (data['marubozu'][i] == -100 or data['longline'][i] == -100) and data['Close'][i] < data['MBB'][i] and data['High'][i] > data['MBB'][i]:
                data['signal'][i] = -100
        ...
        for s in range(len(data)):
            if data['Low'][s] < data['LBB'][s]:
                data['low_bb'][s] = data['Low'][s]
            if data['High'][s] > data['UBB'][s]:
                data['high_bb'][s] = data['High'][s]

        data['low_bb'].replace(0, np.nan, inplace=True)
        data['low_bb'].fillna(method='ffill', inplace=True)

        data['high_bb'].replace(0, np.nan, inplace=True)
        data['high_bb'].fillna(method='ffill', inplace=True)
        ...
        for t in range(len(data)):
            if data['signal'][t] == 100:
                ...
                data['stop_buy'][t] = data['low_bb'][t]
            elif data['signal'][t] == -100:
                data['pr_signal'][t] = data['Close'][t]
                data['stop_sell'][t] = data['high_bb'][t]
